...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  oldpath >>>= 0;
  newpath >>>= 0;
  try {
...
function ___syscall_rmdir(path) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 0, 1, path);
  path >>>= 0;
  try {
    path = SYSCALLS.getStr(path);
    FS.rmdir(path);
...
function ___syscall_symlinkat(target, dirfd, linkpath) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, target, dirfd, linkpath);
  target >>>= 0;
  linkpath >>>= 0;
  try {
...
function ___syscall_truncate64(path, length) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, path, length);
  path >>>= 0;
  length = bigintToI53Checked(length);
  try {
    if (isNaN(length)) return 61;
...
function ___syscall_utimensat(dirfd, path, times, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, dirfd, path, times, flags);
  path >>>= 0;
  times >>>= 0;
  try {
    path = SYSCALLS.getStr(path);
  ...
  registerType(rawType, {
    ...
    "toWireType"(destructors, value) {
      ...
      if (valueIsOfTypeString) {
        ...
        GROWABLE_HEAP_U8().set(value, ptr >>> 0);
      }
      if (destructors !== null) {
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  len >>>= 0;
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 0, 1, addr, len, prot, flags, fd, offset);
  addr >>>= 0;
  len >>>= 0;
  offset = bigintToI53Checked(offset);
  try {
...
function _emscripten_get_element_css_size(target, width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 0, 1, target, width, height);
  target >>>= 0;
  width >>>= 0;
  height >>>= 0;
...
/** @suppress {duplicate } */ function _glGenQueriesEXT(n, ids) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    if (!query) {
      GL.recordError(1282);
      while (i < n) GROWABLE_HEAP_I32()[(((ids) + (i++ * 4)) >>> 2) >>> 0] = 0;
      return;
    }
...
function _emscripten_webgl_get_context_attributes(c, a) {
  ...
  GROWABLE_HEAP_I8()[a >>> 0] = t.alpha;
  GROWABLE_HEAP_I8()[(a) + (1) >>> 0] = t.depth;
  GROWABLE_HEAP_I8()[(a) + (2) >>> 0] = t.stencil;
  GROWABLE_HEAP_I8()[(a) + (3) >>> 0] = t.antialias;
  GROWABLE_HEAP_I8()[(a) + (4) >>> 0] = t.premultipliedAlpha;
  GROWABLE_HEAP_I8()[(a) + (5) >>> 0] = t.preserveDrawingBuffer;
  var power = t["powerPreference"] && webglPowerPreferences.indexOf(t["powerPreference"]);
  GROWABLE_HEAP_I32()[(((a) + (8)) >>> 2) >>> 0] = power;
  GROWABLE_HEAP_I8()[(a) + (12) >>> 0] = t.failIfMajorPerformanceCaveat;
  GROWABLE_HEAP_I32()[(((a) + (16)) >>> 2) >>> 0] = c.version;
  GROWABLE_HEAP_I32()[(((a) + (20)) >>> 2) >>> 0] = 0;
  GROWABLE_HEAP_I8()[(a) + (24) >>> 0] = c.attributes.enableExtensionsByDefault;
  return 0;
}
...
function _environ_get(__environ, environ_buf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 0, 1, __environ, environ_buf);
  __environ >>>= 0;
  environ_buf >>>= 0;
  var bufSize = 0;
  var envp = 0;
  for (var string of getEnvStrings()) {
    ...
    GROWABLE_HEAP_U32()[(((__environ) + (envp)) >>> 2) >>> 0] = ptr;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, penviron_count, penviron_buf_size);
  penviron_count >>>= 0;
  penviron_buf_size >>>= 0;
...
function _fd_close(fd) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 0, 1, fd);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    FS.close(stream);
    return 0;
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 0, 1, fd, pbuf);
  pbuf >>>= 0;
  try {
    var rightsBase = 0;
...
function _fd_seek(fd, offset, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, fd, offset, whence, newOffset);
  offset = bigintToI53Checked(offset);
  newOffset >>>= 0;
  try {
    if (isNaN(offset)) return 61;
...
function _fd_sync(fd) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, fd);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
var __embind_initialize_bindings = Module["__embind_initialize_bindings"] = createExportWrapper("_embind_initialize_bindings", 0);
...
if (typeof exports === 'object' && typeof module === 'object') {
  module.exports = DSA_VS_entry;
  // This default export looks redundant, but it allows TS to import this
  // commonjs style module.
  module.exports.default = DSA_VS_entry;
